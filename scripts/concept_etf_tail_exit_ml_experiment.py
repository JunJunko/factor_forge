...
def main() -> None:
    ...
    print("enriching tail-risk features", flush=True)
    enriched = enrich_exit_features(panel, concepts)
    features = list(config.get("features", EXIT_FEATURES))
    print("building fixed-R4 one-to-two-day tail labels", flush=True)
    states, _ = build_fixed_r4_held_states(
        enriched, start=config["start"], end=config["end"], rules=rules,
    )
    print("fitting purged walk-forward conditional-quantile models", flush=True)
    predictions, fold_importance, fold_audit, folds = fit_tail_walk_forward(
        states, enriched, start=config["start"], end=config["end"],
        feature_columns=features, rules=rules,
    )
    if predictions.empty:
        raise RuntimeError("tail walk-forward produced no OOF predictions")
    tail_audit, oof_states = evaluate_tail_predictions(
        states, predictions, loss_threshold=rules.tail_loss_threshold,
    )
    evaluation_start = pd.Timestamp(predictions["state_date"].min())
    print(f"OOF tail evaluation starts {evaluation_start.date()}", flush=True)
    ...
    for cost in config["roundtrip_cost_bps"]:
        ...
        baseline_periods = evaluation_periods(simulations["T0_fixed"][1], evaluation_start)
        evaluation_daily = []
        ...
        monthly = monthly_performance(combined, benchmark_portfolio="T0_fixed")
    ...
    importance = summarize_tail_importance(fold_importance)
    audit = {
        ...
        "full_sample_tail_event_rate": float(states["tail_event"].mean()),
        "folds_defined": len(folds), "folds_fitted": len(fold_audit),
    ...
    decision = make_decision(summary, audit, config["acceptance"])

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output = Path(args.output_root) / f"concept_etf_tail_exit_ml_{timestamp}"
    output.mkdir(parents=True, exist_ok=False)
    states.to_parquet(output / "held_state_tail_dataset.parquet", index=False)
    predictions.to_parquet(output / "oof_tail_predictions.parquet", index=False)
    oof_states.to_parquet(output / "oof_held_state_tail_audit.parquet", index=False)
    fold_audit.to_csv(output / "fold_audit.csv", index=False, encoding="utf-8-sig")
    fold_importance.to_csv(output / "fold_feature_importance.csv", index=False, encoding="utf-8-sig")
    ...
    exits.to_csv(output / "tail_exit_events.csv", index=False, encoding="utf-8-sig")
    ...
    (output / "research_report.md").write_text(
        render_report(summary, monthly, sleeve_stats, importance, audit, decision, manifest),
        encoding="utf-8",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="One-to-two-day ETF tail-risk exit experiment")
    parser.add_argument("--config", default="configs/research/concept_etf_tail_exit_ml_v1.yaml")
    parser.add_argument("--output-root", default="artifacts/concept_etf_tail_exit_ml")
...
def rules_from_config(config: dict) -> ExitMLRules:
    ...
    return ExitMLRules(
        ...
        consecutive_negative_days=int(execution["confirmation_days"]),
        early_exit_cost_bps=float(execution["early_exit_cost_bps"]),
        tail_horizon_days=int(label["horizon_trading_days"]),
        tail_loss_threshold=float(label["loss_threshold"]),
        tail_quantile=float(label["conditional_quantile"]),
    )
...
def summarize_policy(policy, cost, daily, stats, exits, evaluation_start) -> dict:
    drawdown = daily["net_nav"] / daily["net_nav"].cummax().clip(lower=1.0) - 1
    fifth_percentile = daily["net_return"].quantile(0.05)
    expected_shortfall = daily.loc[daily["net_return"].le(fifth_percentile), "net_return"].mean()
    relevant_exits = exits.loc[exits["exit_date"].gt(evaluation_start)] if not exits.empty else exits
    tail_precision = float(relevant_exits["tail_event_after_exit"].mean()) if not relevant_exits.empty else 0.0
    return {
        ...
        "worst_daily_return": float(daily["net_return"].min()),
        ...
        "realized_tail_precision": tail_precision,
    }


def make_decision(summary: pd.DataFrame, audit: dict, acceptance: dict) -> dict:
    indexed = summary.set_index(["policy", "roundtrip_cost_bps"])
    checks = {}
    for cost in (20, 40):
        tail = indexed.loc[("T1_tail_ml", cost)]
        fixed = indexed.loc[("T0_fixed", cost)]
        placebo = indexed.loc[("T2_tail_placebo", cost)]
        checks[f"drawdown_improvement_{cost}bps"] = bool(
            tail["maximum_drawdown"] - fixed["maximum_drawdown"]
            >= float(acceptance["minimum_drawdown_improvement"])
        )
        checks[f"return_drag_{cost}bps"] = bool(
            fixed["total_return"] - tail["total_return"]
            <= float(acceptance["maximum_total_return_drag"])
        )
        checks[f"expected_shortfall_{cost}bps"] = bool(
            tail["expected_shortfall_5pct"] >= fixed["expected_shortfall_5pct"]
        )
        checks[f"beats_placebo_drawdown_{cost}bps"] = bool(
            tail["maximum_drawdown"] > placebo["maximum_drawdown"]
        )
    precision_floor = max(
        float(acceptance["minimum_tail_precision"]),
        audit["tail_event_base_rate"] * float(acceptance["minimum_precision_lift_over_base"]),
    )
    checks["oof_tail_precision"] = bool(audit["tail_precision"] >= precision_floor)
    checks["oof_tail_recall"] = bool(audit["tail_recall"] >= float(acceptance["minimum_tail_recall"]))
    return {
        "verdict": "KEEP_TAIL_EXIT_FOR_FORWARD_SHADOW" if all(checks.values()) else "TAIL_EXIT_NOT_VALIDATED",
...
def render_report(summary, monthly, sleeve_stats, importance, audit, decision, manifest) -> str:
    summary_display = summary.copy()
    percentage_columns = [
        "total_return", "maximum_drawdown", "worst_daily_return", "expected_shortfall_5pct",
        "mean_daily_turnover", "mean_cash_weight", "minimum_sleeve_excess",
        "median_sleeve_excess", "realized_tail_precision",
    ]
    ...
    importance_display["importance_share"] = importance_display["importance_share"].map(
        lambda value: f"{value:.2%}"
    )
    return f"""# ETF未来1–2日尾部风险退出：走步验证

## 结论

**{decision['verdict']}**。标签只预测从次日开盘开始的未来1–2日最差开盘收益；预测10%条件分位数低于−3%时，次日开盘退出并持有现金至原计划再平衡。

- OOF区间：{manifest['evaluation_start']} 至 {manifest['evaluation_end']}
- OOF尾部事件基准率：{audit['tail_event_base_rate']:.2%}
- 模型事件率：{audit['predicted_event_rate']:.2%}
- 模型精确率/召回率：{audit['tail_precision']:.2%} / {audit['tail_recall']:.2%}
- 安慰剂精确率/召回率：{audit['placebo_precision']:.2%} / {audit['placebo_recall']:.2%}
